...
def test(model, test_loader, args):
    ...
    # 1. Compute Standard Accuracy
    all_acc, old_acc, new_acc = log_accs_from_preds(y_true=targets, y_pred=preds, mask=mask,
                                                    T=0, eval_funcs=args.eval_funcs, save_name='Evaluation')
                                                    
    # 2. Bootstrap Statistical Significance Testing
    print("\n" + "="*50)
    print("Running Bootstrap Statistical Significance Test...")
    print("="*50)
    
    # Bootstrap over All, Old, and New category partitions

    mask_old = mask.astype(bool)
    mask_new = ~mask_old
    
    # All Classes Bootstrap
    mean_all, lower_all, upper_all = bootstrap_accuracy_ci(targets, preds, n_iterations=1000)
    print(f"All Classes Accuracy (95% CI): {mean_all*100:.2f}% [{lower_all*100:.2f}%, {upper_all*100:.2f}%]")
    
    # Old Classes Bootstrap
    if np.sum(mask_old) > 0:
        mean_old, lower_old, upper_old = bootstrap_accuracy_ci(targets[mask_old], preds[mask_old], n_iterations=1000)
        print(f"Old Classes Accuracy (95% CI): {mean_old*100:.2f}% [{lower_old*100:.2f}%, {upper_old*100:.2f}%]")
        
    # New Classes Bootstrap
    if np.sum(mask_new) > 0:
        mean_new, lower_new, upper_new = bootstrap_accuracy_ci(targets[mask_new], preds[mask_new], n_iterations=1000)
        print(f"New Classes Accuracy (95% CI): {mean_new*100:.2f}% [{lower_new*100:.2f}%, {upper_new*100:.2f}%]")
