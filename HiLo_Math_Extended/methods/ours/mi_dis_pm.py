...
# ----------------------
# INIT (Device Agnostic)
# ----------------------
args = parser.parse_args()
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
args.device = device # Store device in args for global access
...
def train(model, dom_head, con_mlp, train_loader, optimizer, exp_lr_scheduler, cluster_criterion, mi_criterion, uda_criterion, mem_fea, mem_cls, epoch, args):
    class_weight_src = torch.ones(args.num_labeled_classes, ).to(args.device)
    ...
    for batch_idx, batch in enumerate(tqdm(train_loader)):
        ...
        class_labels, mask_lab, uq_idxs = class_labels.to(args.device, non_blocking=True), mask_lab.to(args.device, non_blocking=True).bool(), uq_idxs.to(args.device, non_blocking=True)
        images = torch.cat(images, dim=0).to(args.device, non_blocking=True)
        dom_labels = torch.zeros_like(class_labels[mask_lab]).to(args.device)

        img_idx = torch.cat([uq_idxs[~mask_lab] for _ in range(2)], dim=0).to(args.device, non_blocking=True)
        ...
        with torch.cuda.amp.autocast(args.fp16_scaler is not None):
            total_loss, semantic_feats, domain_feats, t_cls, t_logits, t_alpha = model.forward_features(images, mask_lab, label_source, mem_fea=mem_fea, \
