...
from stat_utils import get_dirichlet_uncertainty # Import Dirichlet Prior Module
...
def mixup_unsup_dis(preds, lam):
    ...
    """
    label = torch.eye(preds.shape[0]).to(preds.device)
    mixup_loss = -torch.sum(label* F.log_softmax(preds), dim=1)
    mixup_loss = torch.sum(torch.mul(mixup_loss, lam))
    return mixup_loss
...
class PMTrans(nn.Module):
    ...
    def forward(self, mixture, mask_lab=None, infer_label=None, mem_fea=None, mem_cls=None, img_idx=None, class_weight_src=None):
        ...
        if self.training:
            ...
            # [Core Modification: Dirichlet Prior (Evidential Deep Learning)]
            # Original: t_cls = t_pred.softmax(dim=-1)
            # Replaced with Dirichlet-based probabilities and second-order uncertainty
            t_cls, t_uncertainty, t_alpha = get_dirichlet_uncertainty(t_pred)
            
            dis = -torch.mm(t_feats.detach(), mem_fea.t())
            for di in range(dis.size(0)):
                dis[di, img_idx[di]] = torch.max(dis)
            _, p1 = torch.sort(dis, dim=1)
            w = torch.zeros(t_feats.size(0), mem_fea.size(0)).to(t_feats.device)
            ...
            # [Return t_alpha for external EDL Loss computation]
            return total_loss, student_proj, student_out, t_cls, t_feats, weight_src, t_alpha
        else:
    ...
    def forward_features(self, mixture, mask_lab=None, infer_label=None, mem_fea=None, mem_cls=None, img_idx=None, class_weight_src=None, cluster_criterion=None, epoch=None, args=None):
        ...
        if self.training:
            ...
            # [Return t_alpha for external EDL Loss computation]
            return total_loss, semantic_feats, domain_feats, t_cls, t_feats, t_alpha
        else:
            return student_proj, student_out